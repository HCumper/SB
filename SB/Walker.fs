...
let WalkDim (context: IParseTree) =
    ...
    let paramList =
        context.GetChild(2).Payload :?> SBParser.ParenthesizedlistContext

    let (fList: IParseTree list) =
        copyAntlrList paramList.children
...
let WalkLocal (context: IParseTree) =
    ...
    let paramList =
        context.GetChild(1).Payload :?> SBParser.UnparenthesizedlistContext
...
let WalkAssignment (context: IParseTree) =
    let lvalue =
        context.GetChild(0).GetChild(0).GetText ()
    ...
    let dimensions =
        ...
        | _ ->
            let paramList =
                context.GetChild(0).GetChild(1).Payload :?> SBParser.ParenthesizedlistContext
    ...
    let targetDimensions =
        ...
        | _ ->
            let paramList =
                context.GetChild(2).GetChild(0).GetChild(
                    0
                )
                    .GetChild(
                    1
                    ...
                    .Payload
                :?> SBParser.ParenthesizedlistContext
...
let WalkProcFunc (context: IParseTree) =
    let procName =
        context.GetChild(1).GetChild(0).GetText ()
...
let WalkImplicit (context: IParseTree) =
    ...
    let stringValues =
        List.map (fun (x: IParseTree) -> x.GetText ()) termList
...
let WalkIf (context: IParseTree) =
    let termList =
        ...
        |> List.filter (fun x ->
            not (
                x :? TerminalNodeImpl
                || x :? SBParser.EndIfContext
                || x :? SBParser.LineNumberContext
            ))
