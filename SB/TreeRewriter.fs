...
let rec private WalkAcross (contextList: IParseTree List) =
    let rec WalkAcrossInner index (astChildren: Node List) =
        let count = contextList.Length

        match index with
        | n when n < count ->
            let ast =
                WalkDown (contextList[index]: IParseTree)
...
and translateLineNumber _ = Empty
...
and translateAssignment (context: IParseTree) : SubTree =
...
// top level only
let RewriteTree (parseTree: IParseTree) : SubTree = WalkDown parseTree
