...
let get name scope state =
    ...
    let symbolFound =
        state.symTab
        |> Map.tryFind { Name = name; Scope = scope }
...
let set (entry: Symbol) state =
    ...
    let newTable: Map<Key, Symbol> =
        state.symTab |> Map.add key entry
...
let trySet (entry: Symbol) state =
    ...
    | None -> set entry state
    | Some _ -> state

// create list of eveything in the scope
let listScope currentScope state =
    state.symTab
    |> Map.filter (fun n _ -> n.Scope = currentScope)
...
let testTable =
    ...
    let entry =
        { Name = "c"
          Scope = "func1"
          Category = CategoryType.Function
          Type = SBParser.Implic
          ParameterMechanism = Inapplicable }
