...
let get name scope state =
    ...
    | None ->     // not found so try global
        let newKey = {Name=name; Scope=globalScope}
...
let testTable =
    let state = { references = Set.empty; symTab = Map.empty; errorList = []; currentScope = globalScope; outputProcFn = ""; outputGlobal = ""}
    ...
    let entry = { Name="b"; Scope = globalScope; Category=CategoryType.Procedure; Type=SBParser.Integer;  ParameterMechanism = Inapplicable; }
    ...
    let key2 = { Name="b"; Scope = globalScope;}
    let y = get  "b" globalScope table3
